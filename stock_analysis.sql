...
monthly_summary AS (
    SELECT
      CASE --- Request penyesuaian historikal sales distri lama ke baru
        WHEN UPPER(sti.distributor_name) = 'PT OMEGA SUKSES ABADI' THEN 'PT TRIKARSA RAYA MANDIRI' --Mba Vava
        WHEN UPPER(sti.distributor_name) = 'PT OMEGA SURYA ANUGRAH' THEN 'PT KARYAINDO PUTRA KENCANA' --Mba Vana
    ...
    WHERE sti.calendar_date BETWEEN '2025-08-01' AND '2025-10-31'
    GROUP BY
      distributor_name,
      item_id,
      brand_of,
...
l3m_st AS (
    SELECT
      ...
      COALESCE(total_l3m_st_qty * 1.0 / 3, 0) AS avg_l3m_st_qty
    FROM final_summary
),
...
in_transit AS (
  ...
  LEFT JOIN stock_date sd
    ON TRIM(UPPER(p.distributor_name)) = TRIM(UPPER(sd.distributor))
  WHERE 
    p.sign_off_time IS NULL
    OR DATE(p.sign_off_time) >= sd.current_stock_date
),

in_transit_agg AS (
  SELECT
    TRIM(UPPER(distributor_name)) AS distributor_name,
    sku,
    SUM(in_transit_qty) AS in_transit_qty,
    'IN TRANSIT' AS tagging
  FROM in_transit
  GROUP BY 1,2
),
...
stock_combine AS (
  SELECT
    ...
    oh.stock AS stock,
    tagging
  FROM gt_schema.gt_raw_data_stock oh
  WHERE
    tagging = 'CURRENT STOCK'
  ...
  SELECT
    it.distributor_name,
    it.sku,
    it.in_transit_qty AS stock,
    tagging
  FROM in_transit_agg it
),
...
stock_data AS (
    SELECT
      ...
      SUM(CASE WHEN tagging = 'CURRENT STOCK' THEN sa.stock ELSE 0 END) +
      SUM(CASE WHEN tagging = 'IN TRANSIT' THEN sa.stock ELSE 0 END) AS total_stock
    FROM stock_combine sa
...
main_data AS (
    SELECT
      ...
      COALESCE(sist.avg_l3m_st_qty, 0) AS last_month_st_qty,
      COALESCE(sist.avg_l3m_st_qty, 0) * dp.price_for_store AS last_month_st_value,
...
#cek duplikasi join main table dan stock table --- ada duplikasi--
-- SELECT 
--   dp.distributor,
--   dp.sku AS dp_sku,
--   sd.prod_id AS stock_sku,
--   COUNT(*) AS row_count
-- FROM distri_product dp
-- LEFT JOIN stock_data sd
--   ON UPPER(dp.distributor) = UPPER(sd.distributor)
...
-- SELECT *
-- FROM stock_data
-- WHERE upper(distributor) = 'CV FNB PRATAMA'
--   AND prod_id = 'G2G-04';
...
--   -- sd.distributor,
--   -- sd.prod_id AS sku,
--   sum(sd.in_transit_stock_qty)
-- FROM stock_data sd
-- LEFT JOIN main_data md
--   ON LOWER(sd.distributor) = LOWER(md.distributor)
--  AND LOWER(sd.prod_id) = LOWER(md.sku)
-- WHERE md.sku IS NULL
--   AND sd.prod_id not like 'G2G%'
--   AND sd.in_transit_stock_qty <> 0

-- select *
-- from main_data
-- where brand = 'BODIBREZE'

-- select sum(in_transit_stock_qty)
-- from main_data
...
g2g_stock_adjust AS (
SELECT
  tr.* EXCEPT (current_stock_qty, current_stock_value, total_stock, total_stock_value),
  ...
  (tr.current_stock_qty - COALESCE(ss.st_since_stock_date, 0)) + tr.in_transit_stock_qty AS total_stock,
  ((tr.current_stock_qty - COALESCE(ss.st_since_stock_date, 0)) + tr.in_transit_stock_qty) * tr.price_for_distri AS total_stock_value
...
contrib AS (
  ...
  FROM
    buffer_qty_adj b
  WHERE
    REGEXP_CONTAINS(UPPER(b.brand), r'\bG2G\b')
    OR UPPER(b.brand) LIKE '%G2G%'
  GROUP BY
    1,
    2
),
...
allocation_base AS (
  ...
  WHERE
    DATE_TRUNC(a.calendar_date, MONTH) = DATE '2025-11-01'
),

-- 3. po_npd_mtd & po_npd_mtd_region
po_npd_mtd AS (
  SELECT
    distributor_name,
    region,
...
-- 4. final_calculation: Menggabungkan raw_remaining, classified, dan final_allocation
allocation_calc AS (
  SELECT
    ab.region,
    ab.sku,
    ab.distributor,
    ab.allocation,
    ab.allocation_per_dist,
    COALESCE(po.order_qty, 0) AS po_qty_dist,
    -- Hitung raw_remaining_dist
    (ab.allocation_per_dist - COALESCE(po.order_qty, 0)) AS raw_remaining_dist,
    
    -- Klasifikasi Plus & Minus
    GREATEST(0, (ab.allocation_per_dist - COALESCE(po.order_qty, 0))) AS plus_dist,
    GREATEST(0, -(ab.allocation_per_dist - COALESCE(po.order_qty, 0))) AS minus_dist,
    
    -- Window Functions: Agregasi per Region, SKU (MENGGANTI region_summary)
    SUM(GREATEST(0, (ab.allocation_per_dist - COALESCE(po.order_qty, 0)))) 
      OVER (PARTITION BY ab.region, ab.sku) AS total_plus,
    SUM(GREATEST(0, -(ab.allocation_per_dist - COALESCE(po.order_qty, 0)))) 
      OVER (PARTITION BY ab.region, ab.sku) AS total_minus
...
-- Final SELECT: Menghitung remaining_region dan remaining_allocation_qty
final_allocation AS (
SELECT
  region,
  sku,
  ...
  -- Menghitung remaining_region (MENGGANTI region_remaining)
  GREATEST(0, total_plus - total_minus) AS remaining_allocation_qty_region,
  
  -- Menghitung remaining_allocation_qty (MENGGANTI final_allocation logic)
  CASE
    WHEN total_plus < total_minus THEN 0
    WHEN plus_dist > 0 THEN plus_dist * SAFE_DIVIDE(GREATEST(0, total_plus - total_minus), total_plus)
    ELSE 0
  END AS remaining_allocation_qty
