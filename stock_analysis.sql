...
monthly_summary AS (
    ...
    WHERE sti.calendar_date BETWEEN '2025-05-01' AND '2025-07-31' -- L3M Loncat 1 bulan kebelakang lagi karena belom closing
    GROUP BY
      distributor_name,
      item_id,
      brand_of,
...
max_l6m_ish_st AS(
    ...
    FROM (
        SELECT
          ...
          SUM(sti.quantity)           AS monthly_st
        FROM `pbi_gt_dataset.fact_sell_through_all` sti
        WHERE sti.calendar_date BETWEEN '2025-02-01' AND '2025-04-30'
...
target_data AS (
    ...
    WHERE UPPER(type_subject) = 'SELL IN'
      AND DATE_TRUNC(CAST(calendar_date AS DATE), MONTH) =
            /*dynamic_date*/
      --DATE_TRUNC(CURRENT_DATE(), MONTH) -- Curretn month
            /*backdate*/
...
po_mtd AS (
    ...
    LEFT JOIN gt_schema.master_product pr
      ON po.sku = pr.sku
    WHERE DATE_TRUNC(po.order_date, MONTH) = DATE '2025-08-01' -- Backdate bulan Agustus
...
buffer_gap_adj AS (
  SELECT
    ...
    CASE
      ...
      ELSE
        -- buffer awal + tambahan kebutuhan gap
        COALESCE(bg.buffer_plan_by_lm_qty, 0)
...
po_npd_mtd AS (
  ...
  WHERE DATE_TRUNC(order_date, MONTH) = DATE '2025-09-01' -- Bulan September
